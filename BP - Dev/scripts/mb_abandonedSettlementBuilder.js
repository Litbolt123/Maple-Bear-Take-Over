...
/** Jigsaw script try-first — off until Maple Bear full connected jigsaw villages ship (worldgen test archived). */
...
export function tryPlaceAddonJigsaw(dimension, ruleset, location) {
    ...
    try {
        ...
        if (INCLUDE_FULL_DEVELOPER_TOOLS) {
            avLogBuildLine(
                `Jigsaw place FAILED ${id} @ ${Math.floor(location.x)},${Math.floor(location.y)},${Math.floor(location.z)} — ${err?.message ?? err}`
            );
        }
        return { placed: false };
    }
...
export const LAMP_MARKER_ADJUSTMENT_RADIUS = 6;
/** Horizontal search when probing grid snap (worldgen may shift the structure). */
export const LAMP_MARKER_SEARCH_RADIUS = LAMP_MARKER_ADJUSTMENT_RADIUS + 2;
...
function beginCatalogVoidPhase(job) {
    job.phase = "catalog_void";
    job.catalogVoidCursor = { entryIdx: 0, lx: 0, ly: 0, lz: 0 };
}
 ...
 * @returns {number}
 */
function tickStructureCatalogVoid(job, budget) {
    const manifest = job.catalogManifest;
    if (!manifest?.length) {
        job.phase = "done";
        return 0;
    }
    const result = tickCatalogExportVoidFill(
        job.dimension,
        job.centerX,
        job.centerZ,
        job.y,
    ...
    job.catalogVoidCursor = result.cursor;
    if (result.done) {
...
/**
 * Blocks in exported lamp .mcstructure vertical post (center column).
 * @param {string} typeId
 */
function isLampMarkerColumnBlockId(typeId) {
    if (!typeId || typeId === "minecraft:air") return false;
    if (typeId.includes("fence") && !typeId.includes("gate")) return true;
    if (typeId.includes("_wall")) return true;
    if (typeId === "minecraft:barrel" || typeId === "minecraft:lantern") return true;
    return false;
}

/**
 * Probe one column for a worldgen lamp post signature.
 * @returns {{ ok: boolean, maxRun: number, lowestMarkerY: number|null }}
 */
function probeLampMarkerColumnAt(dimension, cx, cz, hintY) {
    const sy = surfaceY(dimension, cx, cz);
    let yMin;
    let yMax;
    if (hintY != null) {
        const hy = Math.floor(hintY);
        yMin = Math.min(hy - 16, (sy ?? hy) - 6);
        yMax = Math.max(hy + 20, (sy ?? hy) + 14);
    } else if (sy != null) {
        yMin = sy - 6;
        yMax = sy + 14;
    } else {
        return { ok: false, maxRun: 0, lowestMarkerY: null };
    }
    yMin = Math.max(-60, yMin);
    yMax = Math.min(320, yMax);

    let maxRun = 0;
    let run = 0;
    let lowestMarkerY = null;
    try {
        for (let y = yMin; y <= yMax; y++) {
            const b = dimension.getBlock({ x: cx, y, z: cz });
            if (!b) return { ok: false, maxRun: 0, lowestMarkerY: null };
            if (isLampMarkerColumnBlockId(b.typeId)) {
                if (lowestMarkerY === null || y < lowestMarkerY) lowestMarkerY = y;
                run++;
    ...
    if (maxRun < 2) return { ok: false, maxRun, lowestMarkerY };

    const footStart = (lowestMarkerY ?? yMin) - 1;
    try {
        for (let y = footStart; y >= footStart - 4; y--) {
            const below = dimension.getBlock({ x: cx, y, z: cz });
            if (!below || below.typeId === "minecraft:air") continue;
            if (isLampMarkerColumnBlockId(below.typeId)) continue;
            if (isPassThroughForColumnScan(below.typeId)) continue;
            return { ok: true, maxRun, lowestMarkerY };
        }
    } catch {
        return { ok: false, maxRun, lowestMarkerY };
    }
    return { ok: false, maxRun, lowestMarkerY };
}

/**
 * Find a worldgen lamp post near grid snap (handles structure_template adjustment_radius shift).
 * @param {import("@minecraft/server").Dimension} dimension
 * @param {number} centerX
 * @param {number} centerZ
 * @param {number} [hintY]
 * @param {number} [searchRadius]
 * @returns {{ x: number, z: number, maxRun: number }|undefined}
 */
export function findWorldgenLampMarkerNear(dimension, centerX, centerZ, hintY, searchRadius = LAMP_MARKER_SEARCH_RADIUS) {
    const icx = Math.floor(centerX);
    const icz = Math.floor(centerZ);
    for (let r = 0; r <= searchRadius; r++) {
        for (let dx = -r; dx <= r; dx++) {
            for (let dz = -r; dz <= r; dz++) {
                ...
                const x = icx + dx;
                const z = icz + dz;
                const probe = probeLampMarkerColumnAt(dimension, x, z, hintY);
                if (probe.ok) return { x, z, maxRun: probe.maxRun };
            }
        }
    }
    return undefined;
...
/**
 * True when a worldgen lamp post exists near grid snap coords.
 * @param {import("@minecraft/server").Dimension} dimension
 * @param {number} worldX
 * @param {number} worldZ
 * @param {number} [hintY]
 * @returns {boolean}
 */
export function hasWorldgenLampMarkerAt(dimension, worldX, worldZ, hintY) {
    return findWorldgenLampMarkerNear(dimension, worldX, worldZ, hintY) !== undefined;
}
...
function tickBuildJob(job, budget) {
    ...
    if (job.phase === "catalog_void") {
        spent += tickStructureCatalogVoid(job, budget - spent);
        if (spent >= budget) return;
    }
...
export function enqueueSettlementBuild(dimension, center, ruleset, tier, cx, cz, onComplete, enqueueOpts = {}) {
    ...
    const job = {
        ...
        catalogVoidCursor: { entryIdx: 0, lx: 0, ly: 0, lz: 0 },
        catalogSignIndex: 0,
